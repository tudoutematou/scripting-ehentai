...
export async function runSelfTests(options: SelfTestOptions = {}): Promise<SelfTestResult[]> {
  const checks: Check[] = [
    ...
    { name: "favorites.fixture-parser", run: () => { const html = `<div class="ido">${Array.from({ length: 10 }, (_, i) => `<div class="fp"><a href="favorites.php?favcat=${i}">${i + 1} Category ${i}</a></div>`).join("")}<a id="uprev" href="favorites.php?favcat=1&page=1">prev</a><a id="unext" href="favorites.php?favcat=1&page=3">next</a><table><tr><td class="glname"><a href="/g/123/abcdef/">Favorite title</a></td><td class="cn">Manga</td><td>12 pages</td></tr></table></div>`; const categories = parseFavoriteCategories(html); const parsed = parseSearchHtml(html, BASE); assert(categories.length === 10 && categories[3].name === "Category 3" && categories[3].count === 4 && parsed.items.length === 1 && Boolean(parsed.prevHref) && Boolean(parsed.nextHref), "收藏真实结构或分页解析错误") } },
