...
import { deleteHistory, deleteSavedSearch, historySummary, loadHistory, loadPreferences, loadSavedSearches, parseDownloads, recordHistory, resumeIndex, savePreferences, saveSearch, updateReadingProgress, type HistoryStore } from "./libraryStore"
...
export async function runSelfTests(options: SelfTestOptions = {}): Promise<SelfTestResult[]> {
  const checks: Check[] = [
    ...
    { name: "parser.my-tags", run: () => { const tags=parseMyTagsHtml('<div id="usertags_outer"><div></div><div id="usertag1"><span id="tagpreview1" title="female:test"></span></div></div>',BASE);assert(tags.length===1&&tags[0].name==="female:test"&&new URL(tags[0].searchUrl).searchParams.get("f_search")==="female:test","My Tags 真实结构解析错误") } },
