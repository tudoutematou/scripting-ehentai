import { buildFavoritesUrl, parseFavoriteCategories } from "./favorites"
...
export async function runSelfTests(options: SelfTestOptions = {}): Promise<SelfTestResult[]> {
  const checks: Check[] = [
    ...
    { name: "favorites.url-parser", run: () => { const url = new URL(buildFavoritesUrl(BASE, 3, { query: "x", searchTags: true, searchNote: true })); assert(url.searchParams.get("favcat") === "3" && url.searchParams.get("f_search") === "x" && url.searchParams.get("f_stags") === "1", "收藏 URL 构造错误"); const categories = parseFavoriteCategories(Array.from({ length: 10 }, (_, i) => `<a href="favorites.php?favcat=${i}">${i + 1} Cat ${i}</a>`).join("")); assert(categories.length === 10 && categories[3].name === "Cat 3" && categories[3].count === 4, "收藏分类解析错误") } },
    { name: "history.progress-store", run: async () => { let raw: string | null = null; const store: HistoryStore = { read: async () => raw, write: async value => { raw = value } }; const item: any = { id: "1:abcdef", gid: "1", token: "abcdef", url: "https://e-hentai.org/g/1/abcdef/", title: "Title", category: "Manga", thumb: "", posted: "", uploader: "u", pages: 12 }; await recordHistory(item, store); await updateReadingProgress(item, 4, store); const history = await loadHistory(store); assert(history.length === 1 && history[0].lastPageIndex === 4 && resumeIndex(history[0], 12) === 4 && historySummary(history[0]).title === "Title", "历史或进度保存错误"); await deleteHistory("1", "abcdef", store); assert((await loadHistory(store)).length === 0, "历史删除错误") } },
    ...
    { name: "library.action-privacy", run: () => { const output = JSON.stringify({ galleryRef: "gallery_x_y", title: "Title" }); assert(!/https?:|\/g\/|abcdef|\"gid\"|\"token\"/i.test(output), "Library action 输出泄漏身份") } },
