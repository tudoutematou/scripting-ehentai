...
export function selectGallerySearchTag(state:GallerySearchState,tag:GallerySearchTag){return composeGallerySearchState(state,"",[...(state.selectedTags||[]),tag])}
...
export function galleryLanguageLabel(language:string){const raw=String(language||"").trim().toLowerCase();return raw?localizeCommonTag("language",raw)||raw:""}
