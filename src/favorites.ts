...
export async function loadFavorites(category?: number, search: FavoriteSearch = {}, directUrl?: string, signal?:AbortSignal): Promise<FavoritesPage> { if (!getAccountStatus().loggedIn) throw new Error("请先登录后查看收藏。"); if (category != null && (!Number.isInteger(category) || category < 0 || category > 9)) throw new Error("收藏分类必须为 0 到 9。"); const url = directUrl && validPageUrl(directUrl) ? directUrl : buildFavoritesUrl(getBaseUrl(), category, search); let response: any; try { const result = await fetchHtml(url, "favorites.list", "", signal); response = result.response; const login = favoriteLoginError(result.html); if (login) throw new Error(login); const parsed = parseSearchHtml(result.html, result.finalUrl),hasGalleryRows=/\bclass\s*=\s*(["'])[^"']*\bitg\b[^"']*\1/i.test(result.html)&&/\/g\/\d+\/[a-f0-9]+/i.test(result.html); if ((parsed.error || hasGalleryRows&&!parsed.items.length) && !/No hits found/i.test(result.html)) throw new Error("收藏列表解析失败，请稍后重试。"); const data = { categories: parseFavoriteCategories(result.html), items: parsed.items, resultCount: parsed.resultCount, prevHref: parsed.prevHref, nextHref: parsed.nextHref, url: result.finalUrl }; await report({ stage: "favorites.list", ok: true, request: { url: data.url, status: response.status }, notes: `items=${data.items.length}; categories=${data.categories.length}` }); return data } catch (error) { await report({ stage: "favorites.list", ok: false, error, request: { url, status: Number(response?.status || 0) } }); throw error } }
async function loadFavoriteCategories():Promise<FavoriteCategory[]>{const result=await fetchHtml(new URL("favorites.php",getBaseUrl()).toString(),"favorites.categories"),login=favoriteLoginError(result.html);if(login)throw new Error(login);return parseFavoriteCategories(result.html)}
export async function loadFavoriteState(item: GallerySummary): Promise<FavoriteState> { if (!getAccountStatus().loggedIn) throw new Error("请先登录后查看收藏备注。"); const ref=parseGalleryRef(item.url);if(!ref)throw new Error("画廊身份无效，无法读取收藏备注。");const url=new URL("gallerypopups.php",getBaseUrl());url.searchParams.set("gid",ref.gid);url.searchParams.set("t",ref.token);url.searchParams.set("act","addfav");const [result,categories]=await Promise.all([fetchHtml(url.toString(),"favorites.read"),loadFavoriteCategories()]);const login=favoriteLoginError(result.html);if(login)throw new Error(login);return mergeFavoriteCategoryNames(parseFavoritePopupHtml(result.html),categories) }
...
export async function changeFavorite(item: GallerySummary, category: number | null, note = ""): Promise<FavoriteState> { if (!getAccountStatus().loggedIn) throw new Error("请先登录后管理收藏。"); if (category != null && (!Number.isInteger(category) || category < 0 || category > 9)) throw new Error("收藏分类必须为 0 到 9。"); const ref = parseGalleryRef(item.url); if (!ref) throw new Error("画廊身份无效，无法管理收藏。"); const url = new URL("gallerypopups.php", getBaseUrl()); url.searchParams.set("gid", ref.gid); url.searchParams.set("t", ref.token); url.searchParams.set("act", "addfav"); const result = await postForm(url.toString(), { favcat: category == null ? "favdel" : String(category), favnote: String(note || "").slice(0, 200), submit: "Apply Changes", update: "1" }, "favorites.change"); const login=favoriteLoginError(result.html);if(login)throw new Error(login); const verified=verifyFavoriteMutationFromState(await loadFavoriteState(item),category,note); invalidateGalleryCaches(); await report({ stage: "favorites.change", ok: true, request: { url: result.finalUrl, status: Number(result.response.status || 0) }, notes: `operation=${category == null ? "remove" : "set"}` });return verified }
