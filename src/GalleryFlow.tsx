...
import { recordHistory, updateReadingProgress, loadHistory, resumeIndex } from "./libraryStore"
...
function ContinuousReaderImage({page,onVisible}:{page:GalleryPageLink;onVisible:()=>void}){const[resolved,setResolved]=useState<{url:string;referer:string}|null>(null);const[error,setError]=useState("");useEffect(()=>{let cancelled=false;void resolveImagePage(page.pageUrl).then(value=>{if(!cancelled){setResolved({url:value.imageUrl,referer:value.pageUrl});onVisible()}}).catch(()=>{if(!cancelled)setError("图片地址解析失败，请重试")});return()=>{cancelled=true}},[page.pageUrl]);return <VStack spacing={8} frame={{maxWidth:"infinity"}}><Text font="caption" foregroundStyle="secondaryLabel">第 {page.index} 页</Text>{resolved?<CachedReaderImage url={resolved.url} referer={resolved.referer}/>:error?<ErrorText message={error}/>:<ProgressView title="加载图片…" progressViewStyle="circular"/>}</VStack>}
...
function GalleryDetailView({summary}:{summary:GallerySummary}) {
 const [detail,setDetail]=useState<GalleryDetail|null>(null),[resume,setResume]=useState<number|null>(null),[loading,setLoading]=useState(true),[previewsLoading,setPreviewsLoading]=useState(false),[failed,setFailed]=useState<number[]>([]),[error,setError]=useState(""),[retryToken,setRetryToken]=useState(0),[favoriteBusy,setFavoriteBusy]=useState(false)
