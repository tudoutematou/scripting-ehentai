...
import { changeFavorite, parseFavoriteState } from "./favorites"
...
function GalleryDetailView({summary}:{summary:GallerySummary}) {
  const [detail,setDetail]=useState<GalleryDetail|null>(null); const [resume,setResume]=useState<number|null>(null); const [loading,setLoading]=useState(true); const [previewsLoading,setPreviewsLoading]=useState(false); const [failed,setFailed]=useState<number[]>([]); const [error,setError]=useState(""); const [retryToken,setRetryToken]=useState(0); const [favoriteBusy,setFavoriteBusy]=useState(false)
