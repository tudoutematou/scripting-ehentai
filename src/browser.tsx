...
function normalize(raw:any, fallback:string){return {name:String(raw?.name||""),value:String(raw?.value||""),domain:String(raw?.domain||fallback),path:String(raw?.path||"/"),secure:Boolean(raw?.secure??raw?.isSecure),httpOnly:Boolean(raw?.httpOnly??raw?.isHTTPOnly),session:Boolean(raw?.session??raw?.isSessionOnly),expirationDate:raw?.expirationDate??raw?.expiresDate??null}}
function valid(cookies:any[]){const names=new Set(cookies.map(cookie=>cookie.name));return [...REQUIRED].every(name=>names.has(name))}
