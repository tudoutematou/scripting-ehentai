...
export async function loadPreviewPageBatch(detail: GalleryDetail, startPage?: number, count = 2): Promise<PreviewLoadResult> {
  ...
  const worker = async () => { while (pending.length) { const page = pending.shift(); if (page == null) return; try { results.set(page, await fetchPreviewPage(withPreviewPage(detail.sourceUrl, page), page)); loaded.add(page); failed.delete(page) } catch { failed.add(page) } } }
...
export async function loadPreviewPageRange(detail: GalleryDetail, firstPage: number, lastPage: number): Promise<PreviewLoadResult> { const total = Math.max(1, Number(detail.previewPages || 1)), first = Math.floor(firstPage), last = Math.floor(lastPage); if (!Number.isInteger(first) || !Number.isInteger(last) || first < 0 || last < first || last >= total) throw new Error("预览分页范围无效。"); return loadPreviewPageBatch(detail, first, last - first + 1) }
