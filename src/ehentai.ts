...
// Only the abort operations used here; accepts both native Scripting and web signals.
export type RequestAbortSignal = { readonly aborted: boolean; addEventListener(type: "abort", listener: () => void): void; removeEventListener(type: "abort", listener: () => void): void }
...
export async function authenticatedFetch(url:string,init:Record<string,any>={},context:AccountRequestContext=captureAccountRequestContext()):Promise<Response>{if(typeof url!=="string"||!/^https:\/\//i.test(url))throw new Error("请求地址无效。");const headers={...(init.headers||{}),...productionRequestAuth(url,context.cookies).headers},response=await fetch(url,{...init,headers});try{storeResponseCookies(Array.from((response as any).cookies||[]),undefined,context)}catch{}return response}
function requestSignal(signal?:RequestAbortSignal){const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),HTML_REQUEST_TIMEOUT_MS),abort=()=>controller.abort();signal?.addEventListener?.("abort",abort);if(signal?.aborted)controller.abort();return{signal:controller.signal,dispose:()=>{clearTimeout(timeout);signal?.removeEventListener?.("abort",abort)}}}
export async function fetchHtml(url:string,stagePrefix:string,referer="",signal?:RequestAbortSignal,context:AccountRequestContext=captureAccountRequestContext()):Promise<{html:string;finalUrl:string;response:Response}>{
...
export async function searchGalleries(keyword:string,directUrl?:string,signal?:RequestAbortSignal):Promise<SearchPage>{
...
async function fetchPreviewPage(url:string,previewPageIndex:number,signal?:RequestAbortSignal,context:AccountRequestContext=captureAccountRequestContext()):Promise<GalleryPageLink[]>{
...
export function accountOverviewUrl(){return new URL("home.php",getBaseUrl("e")).toString()}
export function accountOverviewUnavailableMessage(){return "账户信息暂时无法加载"}
...
export async function loadAccountOverview():Promise<AccountOverview>{const result=await fetchHtml(accountOverviewUrl(),"account-overview");if(/requires you to log on|not logged in/i.test(result.html))throw new Error("账户概览需要登录。");return parseAccountOverviewHtml(result.html)}
