...
export async function openResource(url:string):Promise<void>{const target=new URL(url);if(target.protocol!=="https:"||!/(?:^|\.)e-hentai\.org$|(?:^|\.)exhentai\.org$/i.test(target.hostname))throw new Error("资源地址无效。");if(!await Safari.openURL(target.toString()))throw new Error("无法打开系统浏览器。")}
