...
function RegularShell({selected,onSelectedChanged}:{selected:RootDestination;onSelectedChanged:(value:RootDestination)=>void}){const[epoch,setEpoch]=useState(0);const choose=(value:RootDestination)=>{if(value===selected)setEpoch(count=>count+1);else{onSelectedChanged(value);setEpoch(0)}};return <NavigationSplitView sidebar={<List navigationTitle="E-Hentai" navigationSplitViewColumnWidth={{min:220,ideal:240,max:280}} selection={{value:selected,onChanged:value=>{if(value)choose(value as RootDestination)}}}><Section>{ROOTS.map(item=><Button key={item.key} tag={item.key} action={()=>choose(item.key)} buttonStyle="plain"><Label title={item.title} systemImage={item.icon}/></Button>)}</Section></List>}><NavigationStack key={regularRootNavigationKey(selected,epoch)}><RootScene value={selected}/></NavigationStack></NavigationSplitView>}
