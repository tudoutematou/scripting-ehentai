...
function RegularShell({selected,onSelectedChanged}:{selected:RootDestination;onSelectedChanged:(value:RootDestination)=>void}){return <NavigationSplitView sidebar={<List navigationTitle="E-Hentai" navigationSplitViewColumnWidth={{min:220,ideal:240,max:280}}><Section>{ROOTS.map(item=><Button key={item.key} action={()=>onSelectedChanged(item.key)} buttonStyle="plain"><Label title={item.title} systemImage={item.icon}/></Button>)}</Section></List>}><NavigationStack><RootScene value={selected}/></NavigationStack></NavigationSplitView>}
function CompactShell({selected,onSelectedChanged}:{selected:RootDestination;onSelectedChanged:(value:RootDestination)=>void}){const tabIndex=ROOTS.findIndex(item=>item.key===selected);return <TabView tabIndex={Math.max(0,tabIndex)} onTabIndexChanged={index=>{const next=ROOTS[index];if(next)onSelectedChanged(next.key)}}><NavigationStack tabItem={<Label title="发现" systemImage="safari"/>} tag={0}><HomeScene/></NavigationStack><NavigationStack tabItem={<Label title="书库" systemImage="books.vertical"/>} tag={1}><LibraryScene/></NavigationStack><NavigationStack tabItem={<Label title="设置" systemImage="gearshape"/>} tag={2}><SettingsRoot/></NavigationStack></TabView>}
function ResponsiveShell(){const[generation,setGeneration]=useState(0),[selected,setSelected]=useState<RootDestination>("discover");useEffect(()=>{const previous=(globalThis as any).__ehAccountContextChanged;(globalThis as any).__ehAccountContextChanged=(value:number)=>{setGeneration(Number(value)||Date.now())};return()=>{(globalThis as any).__ehAccountContextChanged=previous}},[]);return <EnvironmentValuesReader key={generation} keys={["horizontalSizeClass"]}>{environment=>environment.horizontalSizeClass==="compact"?<CompactShell selected={selected} onSelectedChanged={setSelected}/>:<RegularShell selected={selected} onSelectedChanged={setSelected}/>}</EnvironmentValuesReader>}
