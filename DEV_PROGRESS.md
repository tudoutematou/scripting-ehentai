...
- **S0:** A-01, A-02 fixed.
- **S1:** A-03 through A-08 fixed. Independent final review also fixed image-cache HTML/invalid-payload poisoning and unconfirmed Favorite mutation feedback. **No known open S0/S1.**
- **S2:** practical code-backed consolidation complete: session/account lifecycle, stale requests, Watched paging, Toplists identity/rank, parser relations, reader/offline progress, download recovery/reconciliation, storage backup recovery, destructive confirmations, and safe UI errors.
...
- `src/runSelfTests.ts`: **all 53 executed checks passed**.
...
- `src/runNetworkSelfTest.ts`: **passed**; live Search -> Detail Core -> Image Page completed, with all deterministic checks green.
