...
| BS-15 | S1 | 用户在搜索框输入中文标签提示词并从本地标签建议中选择精确标签后，搜索结果只有几十条且大量标题直接含该中文词；从任一详情页点击同一个真实标签却能得到数万条结果 | `SearchComposer.select()` 在把建议转换为 `GallerySearchTag` 时仍调用 `composeGallerySearchState(current,inputRef.current,[...tags,tag])`，先把用于找建议的中文草稿保存为普通 `keyword/rawQuery`，然后才清空输入框；因此 UI 看似只剩 tag chip，实际请求是“中文普通关键词 + 精确标签”。`composeGallerySearchState()` 明确把 plainText 与 `galleryExactTagTerm()` 同时拼进 `rawQuery` | open：选择标签建议必须“消费”当前建议草稿，生成纯 exact-tag search；同一标签通过建议选择与详情标签点击必须产生等价标签查询/同量级真实结果。选中 tag 后用户若再主动输入新的普通文本，才允许组合为 `new plain text + exact tag`。需 focused state/url regression + 真实 DEV 对照搜索 |
