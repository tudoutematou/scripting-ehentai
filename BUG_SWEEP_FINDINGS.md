...
| BS-16 | S1 | 用户打开 1323 页超长画廊时，详情页先在后台持续抓取预览库存，页面预览最终停在 1000；“查看全部/开始阅读”受完整库存门控，导致必须等大量预览分页加载，且超过上限的画廊永远无法被视为完整。录屏明确显示基本信息 1323 页但预览库存固定停在 1000 | 当前详情 mount 后只要 `core.previewPages>1` 就立即 `loadRemainingPreviewPages()`；该函数把所有预览分页一次性并发扫到 `MAX_PREVIEW_LIST_PAGES=50`，而当前服务器每预览分页约 20 个页面链接，因此形成约 1000 页硬上限。`hasCompletePreviewInventory()` 又要求 `!truncatedPreviewPages`，`inventoryReady` 同时要求 `!previewsLoading`，把预览、Reader 和下载错误地绑定到“先把整个画廊库存抓全” | implemented · needs user gesture/visual test：详情只读取 core 首批预览；预览浏览器和 Reader 接近已加载尾部时才请求下一批最多 2 个预览页；50 页上限已删除，显示页数读取服务器 metadata，查看全部/阅读不再等待完整库存。下载仅在用户明确开始后逐批补齐库存并显示准备进度。focused regression 覆盖 1323 页/67 preview-page 的首批、阈值、完整下载门控；真实 DEV 当前 1324 页详情 core 约 1.2 秒返回 20 首批页面，显式一批才扩至 40。需用户在录屏的 1323 页画廊实点查看全部滚至尾部、Reader 跨批次、一次返回链；当前无 App 内 UI 自动化 |
| BS-17 | S2 | 用户从脚本内“AI 助手”要求寻找并推荐某类画廊，实际得到的是通用聊天模型给出的关键词/标签建议，没有真实执行站内搜索，也没有返回候选画廊。截图中“帮我搜索 VR 有关的本子”被回答为一串 VRMMO/フルダイブ 等建议词 | `presentManagedAssistant()` 仅启动/展示 Scripting 通用 Conversation；是否调用 Assistant Tool 完全由模型自行决定。当前 Assistant Tool 虽支持 search/detail/favorites/history，但 `search` 只暴露 query/category/language，没有精确 include/exclude tags、页数、评分等搜索状态；脚本也没有“先真实搜索→取候选安全元数据→AI 排序推荐”的强制编排。因此前台“AI 助手”承诺感强于真实能力 | runtime checked（生产核心）· needs user gesture/visual test：前台已移除 `💬 AI 助手` / `✨ 问 AI`，保留 Assistant Tool 兼容入口；新增 `✨ AI 推荐`：Provider 生成最多 3 个本地校验 SearchState → 必定执行真实 searchGalleries → 稳定去重至 12 个候选 → 仅 loadGalleryDetailCore 安全元数据（不枚举完整预览）→ Assistant 只能以提供的 candidateId 排序 3–5 项 → 原生 Gallery 卡通过既有单 selection 导航。真实 DEV 当前 Provider 输入 VR/虚拟世界偏好，执行 3 个真实搜索、取得 12 个候选并返回 3 个受候选 ID 约束的结果；不得退化为关键词建议。需用户在最终 UI 点 `✨ AI 推荐`，确认实际卡片、详情一层 push 与一次返回 |
