...
| UConfig / site account configuration | 🟡 | Native cloud-favorite-category rename only; broader account configuration is not exposed | 2 |
...
| Cloud Favorites | 🟡 | Server-named categories/counts, note, add/change/remove and category rename with UConfig read-back verification; default/sort and batch move/delete/download remain unimplemented | 2 |
...
| Favorite category rename | ✅ | Native UConfig-backed 10-slot management; submits preserved server settings and requires read-back match before refresh | — |
| Default favorite category | ⚪ | Deferred: current UConfig has no unambiguous default-category field; do not infer one | 2 |
| Favorite category ordering | ⚪ | Deferred: server categories are fixed indices 0–9 and no independent persistent order was established | 2 |
...
| UConfig | 🟡 | Favorite-category rename only; broader UConfig is intentionally not exposed | **2** |
...
1. **Account parity:** make E/Ex login/site state trustworthy and understandable; then expand UConfig only where server protocol is clear.
