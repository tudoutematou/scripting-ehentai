...
- **severity:** S1
- **symptom / user risk:** A worker can recreate files after a confirmed delete, and a manifest-write plus rollback-rename failure can leave content inaccessible in `.deleting`. A later delete can drop the manifest without restoring or cleaning that directory.
- **root cause:** `deleteDownload()` aborts but does not await `runDownload()`; the worker's active-state check and file commit are not atomic. `deleteOfflineDirectoryTransaction()` swallows rollback failure and only recognizes trash when the normal directory also exists.
...
- **severity:** S1
- **symptom / user risk:** Reader can be opened while background preview pages are still loading and receives a permanently partial page array. Failed preview pages have no recovery action. Downloads discard `failedPreviewPages`/truncation information and can mark only the known subset as completed.
- **root cause:** Detail Core exposes reading/download actions before page inventory is complete; `loadGalleryDetail()` loses partial-failure metadata; completion compares `done` only with the partial manifest pages. The 50-preview-page cap can also truncate very large galleries.
...
- **severity:** S1
- **symptom / user risk:** When the account uses a non-table display mode, title/category/thumb/uploader/page count can be taken from adjacent galleries, making Home/Search/Watched/Favorites broadly unreliable.
- **root cause:** `searchHtml.ts` falls back from `<tr>` to a fixed 11 KB character window around each gallery link rather than isolating an actual `.itg` child/item boundary.
...
- **severity:** S1
- **symptom / user risk:** A stalled CDN request can keep a queue permanently `downloading`; an HTTP 200 login/limit/challenge HTML page or empty response is written as `.img`, counted done, and can make an unreadable download appear completed.
- **root cause:** the binary fetch has only the task abort signal and no timeout; success checks only `response.ok` and never validates content type, non-empty data, or image decodability before atomic commit.
...
- **severity:** S1
- **symptom / user risk:** Logout, relogin, site switching or cloud favorite changes can return permanent cached detail from the previous account/session. A stale “not favorited” result can cause a later favorite action to skip loading the existing note and overwrite it with an empty note.
- **root cause:** module-level `detailCoreCache` and `previewPageCache` have no TTL/session generation; account lifecycle and favorite mutations do not invalidate/update them.
